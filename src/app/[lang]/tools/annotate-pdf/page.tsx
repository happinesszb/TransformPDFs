...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: t.tools.annotatePdf.title,
    description: t.tools.annotatePdf.description,
  }
}

export default async function AnnotatePDFPage({ params }: Props) {
  const safeParams = await Promise.resolve(params);
  const locale = await getCurrentLocale(safeParams);
  const t = getTranslations(locale);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-12 text-center">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">
              {t.tools.annotatePdf.title}
            </h1>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              {t.tools.annotatePdf.description}
            </p>
          </div>

    ...
    <footer className="border-t border-gray-200 bg-white">
      <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
            ...
            {isLangMenuOpen && (
              <div className="absolute bottom-full left-0 mb-2 bg-white border border-gray-200 rounded-lg shadow-lg py-2 w-40">
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
                    onClick={() => handleLanguageChange(lang.code as keyof typeof LANGUAGE_PATHS)}
                    className={`w-full text-left px-4 py-2 hover:bg-gray-100 ${
                      locale === lang.code ? 'text-blue-600' : 'text-gray-600'
                    }`}
                  >
                    {lang.label}
                  </button>
        ...
        <div className="flex items-center space-x-6 text-sm text-gray-500">
          ...
          <span>|</span>
          <span>Copyright © 2024 Zbshareware Limited. All rights reserved.</span>
        </div>
      </div>
    </footer>
  );
